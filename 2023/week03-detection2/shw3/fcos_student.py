...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        for level_name, value_shape in dummy_out_shapes:
            self.fpn_params.add_module(f'lateral_{level_name}', nn.Conv2d(value_shape[1], self.out_channels, kernel_size=(1, 1)))
            self.fpn_params.add_module(f'output_{level_name}', nn.Conv2d(self.out_channels, self.out_channels, kernel_size=(3, 3), padding=(1, 1)))
    ...
    def forward(self, images: torch.Tensor):
        ...
        for i, (level_name, value) in enumerate(reversed(backbone_feats.items())):
            x = self.fpn_params.get_submodule(f'lateral_{level_name}')(value)
            if prev_additional is not None:
                x = F.interpolate(prev_additional, size=value.shape[2:], mode="nearest") + x
            prev_additional = x.clone()
            fpn_feats[f"p{5 - i}"] = self.fpn_params.get_submodule(f'output_{level_name}')(x)
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        location_coords[level_name] = ((torch.stack([x.reshape(-1), y.reshape(-1)]).T + 0.5) * level_stride).to(dtype=dtype, device=device)  # type: ignore[code]
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(stem_channels)):
            stem_cls.append(nn.Conv2d(in_channels if i == 0 else stem_channels[i - 1], stem_channels[i], kernel_size=(3, 3), padding=(1, 1)))
            stem_box.append(nn.Conv2d(in_channels if i == 0 else stem_channels[i - 1], stem_channels[i], kernel_size=(3, 3), padding=(1, 1)))
            if i != len(stem_channels) - 1:
        ...
        for stem in self.stem_cls:
            if isinstance(stem, nn.Conv2d):
                torch.nn.init.normal_(stem.weight, mean=0, std=0.01)
                torch.nn.init.constant_(stem.bias, 0)
        
        for stem in self.stem_box:
            if isinstance(stem, nn.Conv2d):
                torch.nn.init.normal_(stem.weight, mean=0, std=0.01)
                torch.nn.init.constant_(stem.bias, 0)
        ...
        self.pred_cls = nn.Conv2d(stem_channels[-1], num_classes, kernel_size=(3, 3), padding=(1, 1))  # Class conv
        self.pred_box = nn.Conv2d(stem_channels[-1], 4, kernel_size=(3, 3), padding=(1, 1))  # Box regression conv
        self.pred_ctr = nn.Conv2d(stem_channels[-1], 1, kernel_size=(3, 3), padding=(1, 1))  # Centersness conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, value in feats_per_fpn_level.items():
            x_cls = self.stem_cls(value)
            x_box = self.stem_box(value)

            y_cls = torch.flatten(self.pred_cls(x_cls), start_dim=2, end_dim=3).transpose(1, 2)
            y_box = torch.flatten(self.pred_box(x_box), start_dim=2, end_dim=3).transpose(1, 2)
            y_ctr = torch.flatten(self.pred_ctr(x_box), start_dim=2, end_dim=3).transpose(1, 2)

            class_logits[level_name] = y_cls
            boxreg_deltas[level_name] = y_box
            centerness_logits[level_name] = y_ctr
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    bad_idx = gt_boxes == -torch.ones((4,)).to(gt_boxes.device)
    
    locations_copy = -locations.clone()
    all_locations = torch.cat((locations, locations_copy), dim=1)

    gt_boxes = gt_boxes * torch.tensor([-1, -1, 1, 1]).to(gt_boxes.device)

    deltas = (all_locations + gt_boxes) / stride
    deltas[bad_idx] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    deltas[deltas < 0] = 0
    deltas = deltas * torch.tensor([-1, -1, 1, 1]).to(deltas.device) * stride
    locations = torch.cat((locations, locations), dim=1)

    output_boxes = deltas + locations
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    bad_idx = deltas == -torch.ones((4, )).to(deltas.device)
    bad_idx = bad_idx[:, 0]
    
    min_1, _ = torch.min(deltas[:, [0, 2]], dim=1)
    min_2, _ = torch.min(deltas[:, [1, 3]], dim=1)
    max_1, _ = torch.max(deltas[:, [0, 2]], dim=1)
    max_2, _ = torch.max(deltas[:, [1, 3]], dim=1)

    centerness = torch.sqrt((min_1 * min_2) / (max_1 * max_2))
    centerness[bad_idx] = -1
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    first_boxes = boxes.unsqueeze(1)
    second_boxes = boxes.unsqueeze(0)

    pairwise_left_angle = torch.maximum(first_boxes[:, :, :2], second_boxes[:, :, :2])
    pairwise_right_angle = torch.minimum(first_boxes[:, :, 2:], second_boxes[:, :, 2:])

    diff = pairwise_right_angle - pairwise_left_angle
    IoU = diff[:, :, 0] * diff[:, :, 1]
    non_intersect = torch.min(diff[:, :, 0], diff[:, :, 1]) < 0
    
    first_S = boxes[:, :2] - boxes[:, 2:]
    first_S = first_S[:, 0] * first_S[:, 1]
    second_S = first_S.reshape(-1, 1)
    IoU = IoU / (first_S + second_S - IoU)
    IoU[non_intersect] = 0

    torch.diagonal(IoU, 0).zero_()

    deleted = set()
    idxs = scores.argsort(descending=True)

    for idx in idxs:
        if idx.item() not in deleted:
            keep.append(idx)
            delete_idxs = set(torch.arange(0, scores.shape[0]).to(boxes.device)[IoU[idx] > iou_threshold].tolist())
            deleted = deleted.union(delete_idxs)
            IoU[:, list(delete_idxs)] = 0
    keep = torch.tensor(keep, dtype=torch.long).to(boxes.device)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        locations_per_fpn_level = get_fpn_location_coords(shape_per_fpn_level=shape_per_fpn_level,
                                                          strides_per_fpn_level=strides_per_fpn_level, device=next(self.parameters()).device)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        matched_gt_deltas = []
        for box in gt_boxes:
            matched_gt_boxes_elem = fcos_match_locations_to_gt(locations_per_fpn_level, strides_per_fpn_level, box)
            matched_gt_boxes.append(matched_gt_boxes_elem)

            matched_gt_deltas.append(
                {
                    'p3': fcos_get_deltas_from_locations(locations_per_fpn_level['p3'], matched_gt_boxes_elem['p3'], strides_per_fpn_level['p3']),
                    'p4': fcos_get_deltas_from_locations(locations_per_fpn_level['p4'], matched_gt_boxes_elem['p4'], strides_per_fpn_level['p4']),
                    'p5': fcos_get_deltas_from_locations(locations_per_fpn_level['p5'], matched_gt_boxes_elem['p5'], strides_per_fpn_level['p5']),
                }
            )
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        index = matched_gt_boxes[:, :, -1].to(torch.int64) + 1
        onehot = torch.cat((torch.zeros(1, self.num_classes), 
                            torch.eye(self.num_classes)), dim = 0).to(device=index.device)

        matched_gt_classes = onehot[index]
        loss_cls = sigmoid_focal_loss(
            inputs=pred_cls_logits, targets=matched_gt_classes
        )
        # print(loss_cls.sum())
        # if loss_cls.sum() > 1000000:
        #     print(matched_gt_classes.sum(axis=1))
        #     raise
        # if loss_cls.sum() == torch.nan:
        #     print(pred_cls_logits)
        #     print(matched_gt_classes)
        #     raise
        ...
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits.flatten(), matched_gt_centerness, reduction="none"
        )
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            m = level_pred_scores.max(dim=1)
            level_pred_classes = m.indices
            level_pred_scores = m.values

            # Step 2:
            # Replace "pass" statement with your code
            good_idx = level_pred_scores >= test_score_thresh
            level_pred_classes = level_pred_classes[good_idx]

            level_pred_scores = level_pred_scores[good_idx]

            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas[good_idx], level_locations[good_idx], self.backbone.fpn_strides[level_name])
            ...
            level_pred_boxes[:, 0][level_pred_boxes[:, 0] > height] = height
            level_pred_boxes[:, 1][level_pred_boxes[:, 1] < 0] = 0
            level_pred_boxes[:, 1][level_pred_boxes[:, 1] > width] = width
            level_pred_boxes[:, 2][level_pred_boxes[:, 2] < 0] = 0
            level_pred_boxes[:, 2][level_pred_boxes[:, 2] > height] = height
            level_pred_boxes[:, 3][level_pred_boxes[:, 3] < 0] = 0
            level_pred_boxes[:, 3][level_pred_boxes[:, 3] > width] = width
