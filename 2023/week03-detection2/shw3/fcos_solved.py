...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        for level_name, feature_shape in dummy_out_shapes:
            level_name = level_name.replace('c', 'p')
            bs, c_in, h, w = feature_shape
            self.fpn_params[f'{level_name}_lateral'] = nn.Conv2d(c_in, out_channels, kernel_size=1)
            self.fpn_params[f'{level_name}_output'] = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, images: torch.Tensor):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        fpn_feats["p5"] = self.fpn_params["p5_output"](
            self.fpn_params["p5_lateral"](backbone_feats["c5"])
        )

        fpn_feats["p4"] = self.fpn_params["p4_output"](
            F.interpolate(fpn_feats["p5"], scale_factor=2, mode='bilinear') +
            self.fpn_params['p4_lateral'](backbone_feats['c4'])
        )

        fpn_feats["p3"] = self.fpn_params["p3_output"](
            F.interpolate(fpn_feats["p4"], scale_factor=2, mode='bilinear') +
            self.fpn_params['p3_lateral'](backbone_feats['c3'])
        )
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        # Replace "pass" statement with your code
        stem_cls = sum([
            [nn.Conv2d(c_in, c_out, kernel_size=3, stride=1, padding=1), nn.ReLU()]
            for c_in, c_out in zip([in_channels] + stem_channels[:-1], stem_channels)
        ], [])
        stem_box = sum([
            [nn.Conv2d(c_in, c_out, kernel_size=3, stride=1, padding=1), nn.ReLU()]
            for c_in, c_out in zip([in_channels] + stem_channels[:-1], stem_channels)
        ...
        self.pred_cls = nn.Conv2d(stem_channels[-1], num_classes, kernel_size=1)  # Class prediction conv
        self.pred_box = nn.Conv2d(stem_channels[-1], 4, kernel_size=1) # Box regression conv
        self.pred_ctr = nn.Conv2d(stem_channels[-1], 1, kernel_size=1)  # Centerness conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        for key, feature_map in feats_per_fpn_level.items():
            stem_box = self.stem_box(feature_map)
            stem_cls = self.stem_cls(feature_map)
            class_logits[key] = self.pred_cls(stem_cls)
            boxreg_deltas[key] = self.pred_box(stem_box)
            centerness_logits[key] = self.pred_ctr(stem_box)
        
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################

        out = [class_logits, boxreg_deltas, centerness_logits]
        out = [{k: v.view(v.shape[0], v.shape[1], -1).permute(0,2,1) for k,v in x.items()} for x in out]
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    xc, yc = locations.split(1, dim=1)
    x1, y1, x2, y2 = gt_boxes[:, :4].split(1, dim=1)
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    l, t, r, b = torch.clip(deltas, min=0).split(1, dim=1)
    xc, yc = locations.split(1, dim=1)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        matched_gt_boxes = [fcos_match_locations_to_gt(locations_per_fpn_level, self.backbone.fpn_strides, gt_box) for gt_box in gt_boxes]
        # Replace "pass" statement with your code
        pass
        ...
        matched_gt_deltas = [
            {
                level: fcos_get_deltas_from_locations(
                    ...
                    matched_gt_box[level],
                    self.backbone.fpn_strides[level]
                ) for level in matched_gt_box.keys()
            }
        ...
        gt_classes = matched_gt_boxes[..., -1] + 1
        gt_classes = F.one_hot(gt_classes.long(), num_classes=self.num_classes + 1).to(dtype=pred_cls_logits.dtype)[..., 1:]
        ...
        # loss_cls = loss_box = torch.tensor([0]).to(device=images.device)

        b, n, _ = matched_gt_deltas.shape
        matched_gt_centerness = fcos_make_centerness_targets(matched_gt_deltas.view(-1, 4)).view(b,n,1)
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits, matched_gt_centerness, reduction="none",
        )
        loss_ctr[matched_gt_centerness < 0] *= 0.0
        # loss_ctr = torch.tensor([0]).to(device=images.device)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            level_pred_classes = level_pred_scores.argmax(dim=-1, keepdim=True)
            level_pred_scores = torch.gather(
                level_pred_scores,
                dim=1,
                index=level_pred_classes
            )

            level_pred_classes.squeeze_()
            level_pred_scores.squeeze_()
            ...
            level_pred_boxes[:, [0,2]] = torch.clip(level_pred_boxes[:, [0,2]], 0, w)
            # level_pred_boxes[:, [1,3]] = torch.clip(level_pred_boxes[:
